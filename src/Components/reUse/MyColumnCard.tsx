...
import InnerSteps from "../Static/InnerSteps";
...
const MyColumnCard: React.FC<iCard> = ({ title, icon, data, but, display }) => {
    ...
    return <CardColumn display={display}>
        <Title>
            <span>{title}</span>
            {icon && <div>
                {
...
const CardColumn = styled.div<{display?:string}>`
